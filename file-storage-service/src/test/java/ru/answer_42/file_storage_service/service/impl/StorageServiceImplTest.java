...
class StorageServiceImplTest {
  ...
  void success_load_all() throws IOException {
    ...
    //When
    when(fileService.findByUserIdAndFilesId(RANDOM_USER_ID, filesId)).thenReturn(desiredFiles);
    Resource resource = storageService.loadAll(RANDOM_USER_ID, filesId);
...
//    //When
//    FileResponseDto fileResponseDto = storageService.store(RANDOM_USER_ID,
//        multipartFile);
//
//    //Then
...
//  }
  ...
  void success_load_as_resource() {
    ...
    byte[] actualResult = storageService.loadAsResource(RANDOM_USER_ID, FILE_TITLE);
  ...
  void thrown_correct_exception_when_file_under_scan() {
    ...
    FileIsUnderScanException thrown = assertThrows(FileIsUnderScanException.class, () -> {
      storageService.loadAsResource(RANDOM_USER_ID, FILE_TITLE);
    });
  ...
  void thrown_correct_exception_when_user_not_own_file() {
    //Given
    String expectedMessage = "File: " + FILE_TITLE + " isn't own " + NON_ACCESS_USER_ID;
    bytes = new byte[]{1};

    //When
    when(file.getFile()).thenReturn(bytes);
    ResourceNotFoundException thrown = assertThrows(ResourceNotFoundException.class, () -> {
      storageService.loadAsResource(NON_ACCESS_USER_ID, FILE_TITLE);
    });
...
//        () -> {
//          storageService.store(RANDOM_USER_ID, multipartFile);
//        });
//
//    //Then
//    String actualMessage = thrown.getMessage();
